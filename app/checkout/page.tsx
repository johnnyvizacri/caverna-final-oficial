      ...
      <div className="max-w-md w-full bg-gray-900 border border-gray-800 rounded-xl p-8 shadow-2xl">
        ...
        <form onSubmit={handlePayment} className="space-y-4">
          <div>
            ...
            <input name="name" required type="text" className="w-full bg-black border border-gray-700 rounded p-3 focus:border-purple-500 outline-none" />
          </div>
          <div>
            <label className="block text-xs font-bold text-gray-500 uppercase mb-1">E-mail</label>
            <input name="email" required type="email" className="w-full bg-black border border-gray-700 rounded p-3 focus:border-purple-500 outline-none" />
          </div>
          <div className="grid grid-cols-2 gap-4">
            <div>
              <label className="block text-xs font-bold text-gray-500 uppercase mb-1">CPF</label>
              <input name="cpf" required type="text" className="w-full bg-black border border-gray-700 rounded p-3 focus:border-purple-500 outline-none" />
            </div>
            <div>
              <label className="block text-xs font-bold text-gray-500 uppercase mb-1">Celular</label>
              <input name="phone" required type="text" className="w-full bg-black border border-gray-700 rounded p-3 focus:border-purple-500 outline-none" />
            </div>
          </div>
          <button disabled={loading} type="submit" className="w-full bg-green-500 hover:bg-green-400 text-black font-black uppercase py-4 rounded mt-4">
            {loading ? 'Processando...' : 'Pagar Agora'}
          </button>
        </form>
        <button onClick={() => window.history.back()} type="button" className="w-full text-center text-gray-500 text-sm mt-4 hover:text-white">Voltar</button>
      </div>
    </div>
  )
}
